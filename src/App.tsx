...
function Execute(props: ExecuteProps) {
  ...
  return <div className="surface-stack">
    <section className="section-block">
      <div className="section-heading"><div><p className="eyebrow">NativeExecPort</p><h2>Run one reviewed profile</h2></div><Badge>one active process</Badge></div>
    ...
    <section className="section-block two-column"><div><div className="section-heading"><div><p className="eyebrow">Controlled Git operation</p><h2>Detached worktree</h2></div></div>
      ...
      {props.worktreePreview && <div className="compact-review"><strong>{props.worktreePreview.target_ref} → {short(props.worktreePreview.target_commit, 12)}</strong><code>{props.worktreePreview.destination}</code><p>{props.worktreePreview.warning}</p><small>Rollback: {props.worktreePreview.rollback.join(" ")}</small><Button onClick={props.createWorktree}>Create after native confirmation</Button></div>}</div>
...
type ReviewProps = { snapshot: RepositorySnapshotDto; evidence: EvidenceItemDto[]; result: NativeExecResultDto | null; assertion: string; setAssertion: (value: string) => void; producer: string; setProducer: (value: string) => void; caveat: string; setCaveat: (value: string) => void; requirement: string; setRequirement: (value: string) => void; selectedArtifacts: string[]; toggleArtifact: (digest: string) => void; includeCheck: boolean; setIncludeCheck: (value: boolean) => void; preview: SubmissionPreviewDto | null; previewDraft: () => void; submitDraft: () => void; importPreview: SubmissionImportPreviewDto | null; chooseImport: () => void; importSubmission: () => void; submissionResult: SubmissionResultDto | null; busy: boolean };
function ReviewDraft(props: ReviewProps) {
  const runtimeReady = props.snapshot.vela.binary?.state === "signed_runtime_baseline";
  return <div className="surface-stack">{!runtimeReady && <section className="inline-refusal"><AlertTriangle /><div><strong>Submission intake degraded</strong><p>Select the exact signed Vela v0.977.1 runtime. Capture and local export remain available.</p></div></section>}
    <section className="section-block review-columns"><div className="draft-form"><div className="section-heading"><div><p className="eyebrow">Ordinary Submission v3</p><h2>Prepare producer draft</h2></div><Badge>pending Proposal only</Badge></div><label className="field"><span>Bounded assertion</span><textarea value={props.assertion} onChange={(event) => props.setAssertion(event.target.value)} placeholder="State one scientifically reusable bounded result" /></label><div className="form-row"><label className="field"><span>Producer attribution</span><input value={props.producer} onChange={(event) => props.setProducer(event.target.value)} placeholder="agent:researcher" /></label><label className="field"><span>Claim type</span><select defaultValue="computational"><option value="computational">computational</option></select></label><label className="field"><span>Replayability</span><select defaultValue="exact"><option value="exact">exact</option></select></label></div><label className="field"><span>Required caveat</span><textarea value={props.caveat} onChange={(event) => props.setCaveat(event.target.value)} placeholder="What this does not establish" /></label><label className="field"><span>Independent verification requirement</span><input value={props.requirement} onChange={(event) => props.setRequirement(event.target.value)} placeholder="Replay the exact Artifact" /></label>
    ...
    <section className="section-block boundary-ledger"><div><strong>Producer</strong><p>Signs the Submission and reports bounded checks.</p></div><div><strong>Repository authority</strong><p>Remains policy-governed and separate from performer kind.</p></div><div><strong>Unavailable here</strong><p>Verification, accept, reject, Decision, Event, and Standing.</p></div></section>
  </div>;
}
...
export default function App() {
  ...
  const [assertion, setAssertion] = useState(""); const [producer, setProducer] = useState("agent:researcher"); const [caveat, setCaveat] = useState(""); const [requirement, setRequirement] = useState(""); const [selectedArtifacts, setSelectedArtifacts] = useState<string[]>([]); const [includeCheck, setIncludeCheck] = useState(true); const [submissionPreview, setSubmissionPreview] = useState<SubmissionPreviewDto | null>(null); const [importPreview, setImportPreview] = useState<SubmissionImportPreviewDto | null>(null); const [submissionResult, setSubmissionResult] = useState<SubmissionResultDto | null>(null);
  ...
  function resetSourceBoundState() { setWorktreePreview(null); setExecPreview(null); setExecResult(null); setEvidence([]); setCapture(null); setExportPreview(null); setSubmissionPreview(null); setImportPreview(null); setSubmissionResult(null); setSelectedArtifacts([]); }
  ...
  function clear() { guard(async () => { const preferences = await workbench.clearRecents(); setBoot((current) => current ? { ...current, preferences } : current); setIdentity(null); setSelected(null); resetSourceBoundState(); setTool(null); }); }
  ...
  async function runExec() { if (!execPreview) return; const id = newRunId(); setRunId(id); setRunning(true); setError(null); try { const result = await workbench.runNativeExec(id, execPreview); setExecResult(result); setEvidence([]); setCapture(null); setExportPreview(null); setSelectedArtifacts([]); setSubmissionPreview(null); setImportPreview(null); setSubmissionResult(null); if (selected) { const refreshed = await workbench.inspectRepository(selected.path); if (refreshed.git.head_commit !== result.source_commit || refreshed.git.head_tree !== result.source_tree) setExecPreview(null); setSelected(refreshed); } } catch (value) { setError(asError(value)); } finally { setRunning(false); setRunId(null); } }
  ...
  return <div className="app-shell"><aside className="repo-rail"><div className="brand"><div className="brand-mark">V</div><div><strong>Vela</strong><span>Workbench</span></div></div><div className="rail-section"><div className="rail-label"><span>Repositories</span>{(recents.length > 0 || boot?.preferences.vela_binary_path) && <button onClick={clear} aria-label="Clear local repository, tool, run, and evidence choices" title="Clear all local choices"><Trash2 size={13} /></button>}</div><div className="repository-list">{recents.map((path) => <button className="repository-row" data-selected={selected?.path === path || undefined} key={path} onClick={() => inspect(path)} disabled={busy || running}><span className="classification-mark"><FolderGit2 size={15} /></span><span><strong>{path.split("/").slice(-1)[0]}</strong><small>{path.split("/").slice(-2).join("/")}</small></span><ChevronRight size={14} /></button>)}</div><Button variant="secondary" className="choose-button" onClick={choose} loading={busy} disabled={running}><FolderGit2 size={14} />Choose repository</Button></div><div className="rail-footer"><button onClick={selectVela} disabled={busy || running}><Settings2 size={15} /><span><strong>Vela runtime</strong><small>{identity?.version ?? (boot?.preferences.vela_binary_path ? "Signed v0.977.1 selected" : "Choose signed binary")}</small></span></button><p>Tranche 2 · bounded local actions</p></div></aside>
    <main className="workspace">{error && <Refusal error={error} dismiss={() => setError(null)} />}{!selected ? <EmptyState choose={choose} busy={busy} /> : <><header className="source-header"><div className="source-title"><div className="source-icon"><FolderGit2 /></div><div><p className="eyebrow">Selected local source</p><h1>{selected.name}</h1><code>{selected.path}</code></div></div><div className="source-meta"><Badge tone={selected.git.dirty ? "warning" : "positive"}>{selected.git.dirty ? <AlertTriangle size={12} /> : <Check size={12} />}{sourceBadge}</Badge><span><GitBranch size={13} />{short(selected.git.head_commit, 12)}</span><Button variant="ghost" size="sm" onClick={() => inspect(selected.path)} loading={busy} disabled={running}><RefreshCw size={13} />Refresh</Button></div></header>
      <WorkbenchTabs orient={<Orient snapshot={selected} />} execute={<Execute snapshot={selected} launch={launch} launching={launching} worktreeRef={worktreeRef} setWorktreeRef={setWorktreeRef} worktreePreview={worktreePreview} previewWorktree={previewWorktree} createWorktree={createWorktree} profile={profile} setProfile={setProfile} tool={tool} selectTool={selectTool} execPreview={execPreview} previewExec={previewExec} runExec={runExec} cancelExec={cancelExec} running={running} result={execResult} busy={busy} />} capture={<Capture evidence={evidence} result={execResult} selected={capture} setSelected={chooseCapture} chooseFile={chooseEvidence} exclusions={exclusions} setExclusions={setExclusions} redaction={redaction} setRedaction={setRedaction} derivedText={derivedText} setDerivedText={setDerivedText} previewExport={previewEvidenceExport} exportPreview={exportPreview} exportEvidence={exportEvidence} busy={busy} />} review={<ReviewDraft snapshot={selected} evidence={evidence} result={execResult} assertion={assertion} setAssertion={setAssertion} producer={producer} setProducer={setProducer} caveat={caveat} setCaveat={setCaveat} requirement={requirement} setRequirement={setRequirement} selectedArtifacts={selectedArtifacts} toggleArtifact={toggleArtifact} includeCheck={includeCheck} setIncludeCheck={setIncludeCheck} preview={submissionPreview} previewDraft={previewDraft} submitDraft={submitDraft} importPreview={importPreview} chooseImport={chooseImport} importSubmission={importSubmission} submissionResult={submissionResult} busy={busy} />} /></>}</main>
    {boot && <footer className="runtime-footer"><span>Interface <code>{short(boot.runtime.interface_commit)}</code></span><span>Runtime <code>{boot.runtime.runtime_version}</code></span><span>Tranche {boot.runtime.tranche} · no Verification or Decision</span></footer>}</div>;
}
