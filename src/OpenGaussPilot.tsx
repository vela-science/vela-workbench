...
export function OpenGaussPilot({ snapshot, evidence, result }: Props) {
  ...
  return <section className="section-block opengauss-pilot" aria-labelledby="opengauss-title">
    ...
    {receipt && <div className="pilot-receipt">
      ...
      {(receipt.selected_evidence.length > 0 || receipt.selected_checks.length > 0) && <div className="selected-result-list"><strong>Bound exact result facets</strong>{receipt.selected_evidence.map((item) => <div key={item.sha256}><FileCheck2 /><span>{item.display_name}<small>{item.sha256} · {item.size} bytes · {item.source} · source {item.source_commit}/{item.source_tree}</small></span></div>)}{receipt.selected_checks.map((item) => <div key={item.run_id}><Check /><span>{item.run_id}<small>{item.profile} · {item.executable_path} {item.argv.map((value) => JSON.stringify(value)).join(" ")} · cwd {item.working_directory} · source {item.source_commit}/{item.source_tree} · env {item.environment.length} · {item.timeout_ms} ms · capture {item.max_stdout_bytes}/{item.max_stderr_bytes} · {item.producer_check_method}:{item.producer_check_outcome} · stdout {item.stdout_sha256}</small></span></div>)}</div>}
