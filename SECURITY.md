...
- Worktree creation is limited to a native-selected empty destination and one resolved commit through fixed `git worktree add --detach` argv. Source/ref/destination preconditions are repeated, then an OS-native dialog confirms the exact change and rollback command. Workbench never switches or resets the selected checkout.
- Native execution is limited to four app-reviewed profiles with fixed argv and manifest markers. A user-selected tool is canonicalized, hashed before and after use, and never receives ambient credential/signer variables. One process may run at a time; cancellation and timeout kill its process group. Run records are bounded and memory-only.
- Evidence selection reads one regular non-symlink repository-contained file at a time. Exact bytes and SHA-256 are bounded; command output can cross into evidence only through an explicit one-shot export. Export uses a native destination, refuses overwrite, revalidates the source digest, and requires OS-native confirmation.
- Submission author/import runs only pinned `vela submit ... --json`. Rust revalidates source revision and Artifact bytes, shows exact argv or envelope bytes/root, and requires OS-native confirmation. The returned result must report `accepted_event_delta: 0` and `accepted_state_changed: false`; Verification and Decision commands are not compiled into IPC.
