...
- Vela: a user-selected executable must match signed v0.977.1 and its exact macOS arm64 hash. Rust uses only the frozen `submit`, `verification`, `review`, `replay`, `why`, and `recover` JSON surfaces. Only Decision commands receive the ambient SSH agent socket; signer bytes never cross IPC. Vela owns authentication, policy, transaction signing, atomic state and recovery. Performer attribution never grants Repository authority.
