...
The application is a single Tauri 2 desktop binary. A React renderer presents a repository switcher and the Orient → Execute → Capture → Review draft → Verify & Decide journey. The renderer has no filesystem, process, network, signer, or provider API. Private Tauri commands cross only the Rust-owned privilege families below:
...
- Vela: a user-selected executable must match signed v0.977.1 and its exact macOS arm64 hash. Rust uses only the frozen `submit`, `verification`, `review`, `replay`, and `recover` JSON surfaces. Only Decision commands receive the ambient SSH agent socket; signer bytes never cross IPC. Vela owns authentication, policy, transaction signing, atomic state and recovery. Performer attribution never grants Repository authority.
