...
- NativeExec: app-owned profiles map to an exact executable and fixed argv. Selected tools are regular executable files whose canonical path, SHA-256, size, and platform identity are shown. Environments are cleared and rebuilt from a documented non-secret allowlist; stdout/stderr, time, concurrency, and the full process group are bounded. Completed output is ephemeral memory only.
- Evidence: regular non-symlink files must be explicitly selected inside the selected repository; command streams come only from the ephemeral completed-run map. Exact bytes are bounded and returned as UTF-8 plus base64 identity where applicable. Export uses a native destination, create-new semantics, digest recheck, and native confirmation. No directory traversal, repository scan, transcript copy, or background transfer exists.
